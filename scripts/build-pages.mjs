...
<header class="fixed top-0 inset-x-0 z-50 navblur">
  <div class="max-w-[1400px] mx-auto px-5 md:px-10 h-16 flex items-center justify-between">
    <a href="/" class="flex items-center gap-3 group" aria-label="BR Lawn Care home">
      ...
      <div class="leading-none"><div class="display text-[17px]">BR Lawn Care</div><div class="text-[10px] tracking-[0.22em] uppercase text-muted mt-1">Eastern NC</div></div>
    </a>
...
<footer class="border-t border-white/8 py-12 bg-[#080B10]">
  <div class="max-w-[1400px] mx-auto px-6 md:px-10 grid gap-8 md:grid-cols-[1.3fr_1fr_1fr] text-sm text-white/55">
    <div><div class="display text-xl text-white">BR Lawn Care</div><p class="mt-3 max-w-[38ch] leading-relaxed">Locally owned and fully insured lawn, landscape, clearing, tree, exterior-cleaning, and grounds services across Eastern North Carolina.</p></div>
    ...
    <div><div class="font-bold text-white mb-3">Contact</div><div class="grid gap-2"><a href="tel:${site.phoneHref}" class="hover:text-white">${site.phoneDisplay}</a><span>Monday–Saturday, 7 AM–7 PM</span><span>Eastern North Carolina</span></div></div>
  </div>
</footer>`
...
const cityPage = (city, index) => {
  ...
  <section class="max-w-[1100px] mx-auto px-6 md:px-10 pb-24"><div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4"><div><div class="eyebrow mb-3">More service areas</div><h2 class="display text-4xl">Nearby route options.</h2></div><a href="/service-areas" class="btn btn-ghost">View every listed city</a></div><div class="area-link-grid mt-8">${nearby.map((item) => `<a class="area-link-card" href="/service-areas/${item.slug}"><span>${html(item.name)}, NC</span><small>${html(item.county)}</small></a>`).join('')}</div></section>
...
const serviceAreasPage = () => {
  ...
  const description = 'Explore BR Lawn Care service areas across 17 Eastern North Carolina communities. View city coverage, services, and free-estimate guidance.'
  const schema = {
    '@context': 'https://schema.org',
    '@graph': [
  ...
  const body = `<main class="pt-28 md:pt-36">
  <section class="max-w-[1200px] mx-auto px-6 md:px-10 pb-14"><div class="eyebrow mb-5">Eastern NC Service Areas</div><h1 class="display text-5xl md:text-7xl lg:text-[84px]">Lawn care service areas across Eastern North Carolina.</h1><p class="mt-7 text-white/65 text-lg leading-relaxed max-w-[68ch]">BR Lawn Care accepts estimate requests across the communities below. Each city page explains the available service categories and the property details that help the team prepare a useful response.</p><div class="mt-8 flex flex-wrap gap-4"><a href="/contact" class="btn btn-primary">Request a Free Estimate</a><a href="tel:${site.phoneHref}" class="btn btn-ghost">Call ${site.phoneDisplay}</a></div></section>
